...
pub async fn async_conn_with<S: AsyncRead + AsyncWrite + Unpin>(
    stream: &mut S,
    config: &ProxyConfig,
    target_addr: Addr,
    target_port: u16,
    ...
    stream.read_exact(&mut init_resp_buf).await?;
    ...
    stream.write_all(&cmd_req).await?;
...
pub async fn async_create_conn(
    ...
    let (addr, port) = async_conn_with(&mut stream, config, target_addr, target_port).await?;
    Ok((stream, addr, port))
}
